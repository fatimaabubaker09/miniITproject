...
# FIND PARTNER
@app.route("/find_partner", methods=["GET"])
def find_partner():
    if "user_id" not in session:
        return redirect(url_for("login"))

    # Extract filter values from the request
    activity_pref = request.args.get("workouts")
    ...
    # Debugging: Print the filters to ensure they are received
    print("Filters applied:")
    print(f"Activity Preference: {activity_pref}")
    print(f"Frequency: {frequency}")
    print(f"Fitness Level: {level}")
    print(f"Email Search: {email_search}")
    ...
    # Add filters to the query based on user input
    if activity_pref:
